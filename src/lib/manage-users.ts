...
import { confirmDiscardChanges, formSnapshot, isSchoolAdmin, normalizeTeacherAssignmentsForStorage } from './app-config';
...
export function openEditUserModal(userId) {
  ...
  const status = document.getElementById("userFormStatus");
  if (status) { status.hidden = true; status.textContent = ""; }
  toggleTeacherAssignmentFields(u.teacherAssignments || (u.teacherAssignment ? [u.teacherAssignment] : []));
  userFormInitialSnapshot = formSnapshot(document.getElementById("userForm"));
  document.getElementById("userModalBackdrop").classList.add("is-open");
}
...
function updateTeacherAssignmentSummary() {
...
export async function loadTeacherAssignmentSections(selectedSection = "") {
  ...
  const sectionContainer = document.getElementById("u_teacher_section");
  if (!sectionContainer || !year) return;
...
export async function toggleTeacherAssignmentFields(selected = []) {
  ...
  if (!wrapper || !yearSelect || !sectionContainer) return;
  ...
  if (selectAllBtn) {
    selectAllBtn.onclick = () => {
      ...
      updateTeacherAssignmentSummary();
      selectAllBtn.textContent = shouldCheck ? "Clear all" : "Select all";
    };
  }
...
export async function handleUserFormSubmit(e) {
  ...
  try {
    if (record.role === "Teacher") {
      const selectedOptions = [...document.querySelectorAll('#u_teacher_section input[type="checkbox"]:checked')];
      const schoolYear = document.getElementById("u_teacher_year").value.trim();
      record.teacherAssignments = normalizeTeacherAssignmentsForStorage(selectedOptions.map((option) => {
        const selected = JSON.parse(decodeURIComponent(option.value));
        const gradeLevel = normalizeTeacherGrade(selected.gradeLevel);
        const section = String(selected.section || "").trim();
        return { schoolYear, gradeLevel, section, teacherName: record.name };
      }));
      record.teacherAssignment = record.teacherAssignments[0] || null;
      if (!record.teacherAssignments.length || !schoolYear) throw new Error("Select at least one teacher grade and section.");
    } else {
      record.teacherAssignment = null;
