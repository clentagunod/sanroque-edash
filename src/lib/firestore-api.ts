...
export function fsTeacherAssignments_() {
  if (!isTeacher()) return null;
  const profile = storedAppProfile() || {};
  const rawAssignments = Array.isArray(profile.teacherAssignments)
    ? profile.teacherAssignments
    : profile.teacherAssignment ? [profile.teacherAssignment] : [];
  const fallbackUid = String(profile.uid || profile.userId || auth.currentUser?.uid || "teacher").trim() || "teacher";
  const assignments = normalizeTeacherAssignmentsForStorage(rawAssignments).map((assignment) => {
    ...
    const teacherKey = fsAdvisoryKey_(assignment.teacherKey || teacherName || fallbackUid || "teacher");
...
export function fsTeacherAssignment_() {
  return fsTeacherAssignments_()?.[0] || null;
}

export function fsTeacherAssignmentsForSchoolYear_(schoolYear) {
  const normalizedYear = fsNormalizeSchoolYear_(schoolYear);
  return (fsTeacherAssignments_() || []).filter((assignment) => fsNormalizeSchoolYear_(assignment.schoolYear) === normalizedYear);
}
...
function fsSubscribeLiveLearners_(schoolYear, onChange, onError) {
  if (isTeacher()) {
    const key = fsLiveKey_("teacher-assigned-learners", schoolYear);
    let stopped = false;
    const refresh = async () => {
      try {
        const learners = await fsGetTeacherScopedLearners_(schoolYear);
        if (!stopped) fsLiveSet_(key, learners, `firestore_teacher_assignments_${schoolYear}`);
      } catch (error) {
        if (!stopped) fsLiveError_(key, fsError_("Live teacher learner updates", error));
      }
    };
    refresh();
    return fsSubscribeLive_(key, () => {
      // Teachers cannot list the full collection under Firestore rules, so the
      // live listener reuses the same exact scoped query path as the initial fetch.
      refresh();
      return () => { stopped = true; };
    }, onChange, onError);
  }
  const key = fsLiveKey_("learners", schoolYear);
  return fsSubscribeLive_(key, () =>
    fsLearnersCollection_(schoolYear).onSnapshot(
...
export async function fsGetMyProfile(user = null) {
  ...
  const load = async () => {
    ...
    const data = doc.data() || {};
    const teacherAssignments = normalizeTeacherAssignmentsForStorage(Array.isArray(data.teacherAssignments) ? data.teacherAssignments : data.teacherAssignment ? [data.teacherAssignment] : []);
    return {
      ...
      teacherAssignment: teacherAssignments[0] || data.teacherAssignment || null,
      teacherAssignments,
    };
  };
  return load();
...
export async function fsAddUser(record) {
  ...
  try {
    ...
    const existingUsers = await db.collection(FS_USERS).where("email", "==", normalizedEmail).limit(1).get();
    ...
    const teacherAssignments = record.role === "Teacher" ? normalizeTeacherAssignmentsForStorage(record.teacherAssignments || (record.teacherAssignment ? [record.teacherAssignment] : [])) : [];
    await db.collection(FS_USERS).doc(userId).set({
      name: record.name,
      email: normalizedEmail,
      role: record.role,
      status: record.status || "Invited",
      teacherAssignment: record.role === "Teacher" ? teacherAssignments[0] || null : null,
      teacherAssignments,
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
    });
    await fsRefreshTeacherAdvisory_(null, { ...record, role: record.role, status: record.status || "Invited", teacherAssignments }, userId);
    try { await creationAuth.signOut(); } catch (signOutError) { /* Secondary auth cleanup is best effort. */ }
    fsInvalidateReadCaches_();
    fsAudit_("USER_ADD", { sheet: "users", recordId: userId, newValue: JSON.stringify({ name: record.name, email: normalizedEmail, role: record.role, status: record.status || "Invited" }) });
    return { userId };
  } catch (error) {
    if (createdUser) {
    ...
    if (error && error.message && error.message.includes("already exists")) throw new Error("A user with this email already exists in the directory.");
...
export async function fsUpdateUser(userId, record) {
  try {
    ...
    await fsRefreshTeacherAdvisory_(before.exists ? before.data() : null, { ...record, teacherAssignments }, userId);
    fsInvalidateReadCaches_();
...
function fsTeacherTargetsFromUser_(user, userId) {
  if (user?.role !== "Teacher" || user?.status !== "Active") return [];
  const assignments = normalizeTeacherAssignmentsForStorage(Array.isArray(user.teacherAssignments) ? user.teacherAssignments : user.teacherAssignment ? [user.teacherAssignment] : []);
  return assignments.map((assignment) => {
    const gradeLevel = fsNormalizeGrade_(assignment.gradeLevel);
    const section = String(assignment.section || "").trim();
    const teacherName = String(assignment.teacherName || user.name || "").trim();
    if (!assignment.schoolYear || !gradeLevel || !section || !teacherName) return null;
    return { schoolYear: fsNormalizeSchoolYear_(assignment.schoolYear), gradeLevel, section, teacherName, teacherKey: fsAdvisoryKey_(assignment.teacherName || teacherName || userId), gradeKey: fsAdvisoryKey_(gradeLevel), sectionKey: fsAdvisoryKey_(section) };
  }).filter(Boolean);
}

async function fsRefreshTeacherAdvisory_(beforeUser, afterUser, userId) {
  // Advisory data is intentionally disabled. The app no longer recreates teacher
  // advisory records once this collection is removed from Firestore.
  return;
}
...
export async function fsDeleteUser(userId) {
  try {
    ...
    await fsRefreshTeacherAdvisory_(profile.exists ? profile.data() : null, { role: "Disabled", status: "Disabled", teacherAssignments: [] }, userId);
    await ref.delete();

    try {
      await LPSApi.deleteAuthUsers([userId]);
    } catch (authError) {
      console.warn("Firebase Auth cleanup failed during user deletion; Firestore profile was still removed.", authError);
    }
...
export async function fsDeleteUsers(userIds) {
  try {
    const profiles = await Promise.all(userIds.map(async (id) => db.collection(FS_USERS).doc(id).get()));
    await Promise.all(profiles.map((profile, index) => fsRefreshTeacherAdvisory_(profile.exists ? profile.data() : null, { role: "Disabled", status: "Disabled", teacherAssignments: [] }, userIds[index])));
    ...
    try {
      ...
      console.warn("Firebase Auth cleanup failed during bulk user deletion; Firestore profiles were still removed.", authError);
    }
...
let fsPublicStatsRefreshTimer_ = null;

export function fsSchedulePublicStatsRefresh_() {
...
export async function fsGetTransferRecords(schoolYear, type = "all", gradeLevel = "", gender = "") {
  const assignments = fsTeacherAssignmentsForSchoolYear_(schoolYear);
  // Teachers only ever see their own assigned grade-sections (from
  // users/{uid}.teacherAssignments); a teacher with no coverage for this
  // school year gets an explicit empty scope, never a school-wide read
  // (which the security rules reject for teachers).
  if (isTeacher() && !assignments.length) return [];
  ...
  const scopedLearners = assignments.length
    ? learners.filter((learner) => assignments.some((assignment) => fsNormalizeValue_(learner.gradeLevel) === fsNormalizeValue_(assignment.gradeLevel)
...
export async function fsGetArchiveRecords(archive, schoolYear) {
  ...
  // (which the security rules reject for teachers).
  if (isTeacher() && !assignments.length) return [];
  const scopedRecords = assignments.length
    ? records.filter((record) => assignments.some((assignment) => fsNormalizeValue_(record.gradeLevel) === fsNormalizeValue_(assignment.gradeLevel)
  ...
  // they live in the ACTIVE Learners collection but still carry dropout flags,
  // so the scoped query must NOT filter by "active" (fsIsActiveLearner_).
  const activeSnapshot = assignments.length
    ? (await Promise.all(assignments.map((assignment) => fsLearnersCollection_(schoolYear)
        .where("gradeLevel", "==", assignment.gradeLevel)
        .where("section", "==", assignment.section)
        .get()))).flatMap((snapshot) => snapshot.docs)
    : (await fsLearnersCollection_(schoolYear).get()).docs;
  const legacyDropouts = activeSnapshot
    .map((doc) => ({ learnerId: doc.id, schoolYear, ...doc.data() }))
    .filter((learner) => fsLearnerArchiveType_(learner) === "dropout");
  const scopedLegacyDropouts = assignments.length
    ? legacyDropouts.filter((record) => assignments.some((assignment) => fsNormalizeValue_(record.gradeLevel) === fsNormalizeValue_(assignment.gradeLevel)
      && fsNormalizeValue_(record.section) === fsNormalizeValue_(assignment.section)))
    : legacyDropouts;
...
export async function fsGetReportsData(schoolYear) {
  ...
  const profileGraph = (fields) => gradeOrder.map((gradeLevel) => ({ label: gradeLevel, value: learners.filter((learner) => fsNormalizeGrade_(learner.gradeLevel) === gradeLevel && fields.some((field) => learner[field])).length })).filter((row) => row.value);
  return {
    ...
    readingProfiles: profileGraph(["bosyCRLA", "mosyCRLA", "eosyCRLA", "bosyPhilIRI", "mosyPhilIRI", "eosyPhilIRI"]),
    mathProfiles: profileGraph(["bosyRMA", "mosyRMA", "eosyRMA"]),
    nutritionProfiles: profileGraph(["bosyNutritionalStatus", "mosyNutritionalStatus", "eosyNutritionalStatus"]),
  };
}
...
export function fsTeacherAssignmentPairsForSchoolYear_(schoolYear) {
  const normalizedYear = fsNormalizeSchoolYear_(schoolYear);
  return (fsTeacherAssignments_() || [])
    .filter((assignment) => fsNormalizeSchoolYear_(assignment.schoolYear) === normalizedYear)
    .map((assignment) => ({
...
export async function fsGetTeacherScopedLearners_(schoolYear) {
  const assignments = fsTeacherAssignmentPairsForSchoolYear_(schoolYear);
  if (!assignments.length) return [];
  const snapshots = await Promise.all(assignments.map((assignment) =>
  ...
  snapshots.forEach((snapshot) => {
    snapshot.docs.forEach((doc) => {
      const learner = { learnerId: doc.id, ...doc.data() };
      if (!fsIsActiveLearner_(learner)) return;
      merged.set(String(learner.learnerId), learner);
    });
  });
  return [...merged.values()].map((learner) => ({
    ...learner,
...
export async function fsGetLearners(schoolYear) {
  if (isTeacher()) {
    return typeof LPSCache === "undefined"
      ? fsGetTeacherScopedLearners_(schoolYear)
      : LPSCache.getOrLoad(`firestore_teacher_assignments_${schoolYear}`, () => fsGetTeacherScopedLearners_(schoolYear), 15000, 60000);
  }
...
export async function fsGetLearner(learnerId, schoolYear) {
  ...
  const yearAssignments = assignments.filter((item) => item.schoolYear === fsNormalizeSchoolYear_(schoolYear));
  if (isTeacher()) {
    const snapshot = await fsLearnersCollection_(schoolYear).doc(learnerId).get();
    if (!snapshot.exists) return null;
    const learner = { ...snapshot.data(), learnerId: snapshot.id, dateAdded: fsLearnerDate_(snapshot.data().dateAdded || snapshot.data().createdAt || snapshot.data().addedAt) };
    const allowed = yearAssignments.some((assignment) =>
      fsNormalizeValue_(fsNormalizeGrade_(learner.gradeLevel)) === fsNormalizeValue_(assignment.gradeLevel)
      && fsNormalizeValue_(String(learner.section || "")) === fsNormalizeValue_(assignment.section)
    );
...
export async function fsAdvisoryTargets_(schoolYear, learner) {
  ...
  const signedInAssignments = fsTeacherAssignmentsForSchoolYear_(schoolYear);
  const matchingAssignedTargets = signedInAssignments.filter((assignment) =>
    fsNormalizeValue_(assignment.gradeLevel) === fsNormalizeValue_(gradeLevel)
    && fsNormalizeValue_(assignment.section) === fsNormalizeValue_(section)
  );
  if (matchingAssignedTargets.length) {
    matchingAssignedTargets.forEach((assignment) => {
      const teacherName = String(assignment.teacherName || storedAppProfile()?.name || "").trim();
      const teacherKey = fsAdvisoryKey_(assignment.teacherKey || teacherName || storedAppProfile()?.uid || storedAppProfile()?.userId || auth.currentUser?.uid || "teacher");
      targets.set(teacherKey, {
        schoolYear,
        gradeLevel,
        section,
        teacherName: teacherName || "Teacher",
  ...
  sections.filter((item) => fsNormalizeValue_(item.gradeLevel) === fsNormalizeValue_(gradeLevel)
    ...
    if (teacherName) {
      const teacherKey = fsAdvisoryKey_(item.teacherKey || teacherName || storedAppProfile()?.uid || "teacher");
      targets.set(teacherKey, { schoolYear, gradeLevel, section, teacherName, teacherKey, gradeKey: fsAdvisoryKey_(gradeLevel), sectionKey: fsAdvisoryKey_(section) });
    }
  });
  (users || []).filter((user) => user.role === "Teacher" && user.status === "Active").forEach((user) => {
    ...
    const teacherName = String(assignment.teacherName || user.name || "").trim();
    if (teacherName) {
      const teacherKey = fsAdvisoryKey_(assignment.teacherKey || teacherName || user.uid || user.userId || "teacher");
      targets.set(teacherKey, { schoolYear, gradeLevel, section, teacherName, teacherKey, gradeKey: fsAdvisoryKey_(gradeLevel), sectionKey: fsAdvisoryKey_(section) });
    }
  });
  return [...targets.values()];
...
/**
 * The advisory learners index is DERIVED data. The single source of truth for
 * teacher access is users/{uid}.teacherAssignments, and learner records live in
 * Learners/{schoolYear}/records — both already enforced by firestore.rules.
 *
 * This index is therefore maintained ONLY when the signed-in user has explicit
 * advisory write access (admin/registrar). A teacher who creates/updates/deletes
 * a learner never touches advisory (the security rules reject those writes), and
 * this helper must never throw so the canonical learner write can never be
 * blocked by the derived index.
 */
export function fsAdvisoryIndexWriteEnabled_() {
  return !isTeacher();
}

export async function fsSyncAdvisoryLearner_(schoolYear, learner, previousLearner = null) {
  if (!fsAdvisoryIndexWriteEnabled_()) return;
  try {
    ...
    previousTargets.forEach((target) => {
      if (!currentKeys.has(target.teacherKey)) {
        operations.push({
          type: "delete",
          ref: fsAdvisoryLearnersCollection_(target).doc(String(previousLearner.learnerId)),
        });
      }
    });
    currentTargets.forEach((target) => operations.push({
      type: "set",
      ref: fsAdvisoryLearnersCollection_(target).doc(String(learner.learnerId)),
      data: {
        ...learner,
    ...
    // A failed advisory sync must never fail the learner write that triggered
    // it — the Learners/{year}/records document is the authoritative record.
    console.warn("Advisory learner index sync skipped.", error);
  }
}
...
export async function fsAddLearner(schoolYear, learner) {
  try {
    ...
    if (!archiveType && fsAdvisoryIndexWriteEnabled_()) {
...
export async function fsDeleteLearner(schoolYear, learnerId) {
  ...
  try {
    ...
    // `previousLearner` (before.data()) is the authoritative record being
    // removed; the advisory sync is guarded + best-effort by design and
    // short-circuits entirely for teachers.
...
export async function fsDeleteLearners(schoolYear, learnerIds) {
  ...
  try {
    ...
    for (const learnerId of targetIds) {
      ...
      // fsAdvisoryIndexWriteEnabled_); teachers never touch it.
      if (snapshot.exists && fsAdvisoryIndexWriteEnabled_()) {
